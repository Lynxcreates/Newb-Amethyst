...
vec4 nlWater(
  ...
  if (fractCposY > 0.0) { // reflection for top plane
    // bump map - aggressive waves
    bump *= 2.0 * disp(tiledCpos * 2.0, t * 2.0) + 0.24 * sin(t * 4.0 + dot(cPos, vec3_splat(NL_CONST_PI)));
    ...
    bump *= 1.0 + sin(3.0 * t + dot(cPos, vec3_splat(NL_CONST_PI)));
