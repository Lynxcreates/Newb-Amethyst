...
vec3 renderEndSky(vec3 horizonCol, vec3 zenithCol, vec3 viewDir, float t) {
  ...
  float n1 = 0.5 + 0.5*sin(5.0*a + t + 20.0*viewDir.x*viewDir.y);
  float n2 = 0.5 + 0.5*sin(5.0*a + 0.5*t + 5.0*n1 + 0.1*sin(40.0*a -4.0*t));

  float waves = 1.0*n2*n1 + 1.0*n1;

  float grad = 0.0 + 0.4*viewDir.y;
  float streaks = waves*(1.0 - grad*grad*grad*grad);
  streaks += (1.0-streaks)*smoothstep(1.0-waves, -2.0, viewDir.y);

  float f = 0.3*streaks + 0.7*smoothstep(2.0, -0.5, viewDir.y);
  float h = streaks*streaks;
  float g = h*h;
  g *= g;

  vec3 sky = mix(zenithCol, horizonCol, f*f);
  sky += (0.9*streaks + 9.0*g*g*g*1.5*h*g*g*g + h*h*h)*vec3(0.0,0.0,0.0);
  sky += 0.4*h*streaks*spectrum(sin(2.0*viewDir.x*viewDir.y+t));
